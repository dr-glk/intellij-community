...
abstract class HmppImportAndHighlightingTests : MultiplePluginVersionGradleImportingTestCase() {
    ...
    //it is flag that should be passed from test config
    val TEST_SCOPE = TestScope.HIGHLIGHTING

    enum class TestScope {
        E2E, IMPORT, HIGHLIGHTING
    }

    class TestBucket1322 : HmppImportAndHighlightingTests() {


        fun doTest(testCase: GradleTestCase)
        {
            if (TEST_SCOPE <= TestScope.IMPORT)
            {
                configureByFiles()
                importProject()
                ...
                if (TEST_SCOPE == TestScope.E2E) checkHighligthingOnAllModules()
            }
            else if (TEST_SCOPE == TestScope.HIGHLIGHTING) {
                /*Here should be project setup without import, it can be reused from AbstractCodeMetaInfoTest.setupProject, but it needs deep refactoring
                So I just create dependency text from TestCase object and check that it's the same as it would be for MultiplatformAnalysisTestGenerated
                */

                val expected = FileUtil.loadFile(java.io.File(testDataDirectory(), "dependencies.txt"))
                val actual = testCase.ProjectSturcture()
        ...
        fun testSimpleTest() {
            ...
            val testCase = GradleTestCase("my-app", assertType = AssertType.assertExhaustiveModuleDependencyList)
                .addModule(Module("commonMain", isSource = true)
                               .moduleTargets(jvm))
                .addModule(Module("commonTest", isSource = false)
                               .moduleTargets(jvm)
                               .moduleDependency("commonMain", DependencyScope.TEST,DepType.DEPENDS_ON))
                .addModule(Module("jvmMain", isSource = true)
                               .moduleTargets(jvm)
                               .moduleDependency("commonMain", DependencyScope.COMPILE,DepType.DEPENDS_ON))
                .addModule(Module("jvmTest", isSource = false)
                               .moduleTargets(jvm)
                               .moduleDependency("commonMain", DependencyScope.TEST,DepType.DEPENDS_ON)
                               .moduleDependency("commonTest", DependencyScope.TEST,DepType.DEPENDS_ON)
                               .moduleDependency("jvmMain", DependencyScope.RUNTIME,DepType.DEPENDS_ON)
                               .moduleDependency("jvmMain", DependencyScope.TEST,DepType.DEPENDS_ON))
            doTest(testCase)
        }
        @Test
        @PluginTargetVersions(pluginVersion = "1.5.0+")
